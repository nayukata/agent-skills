...
- **NEVER**: Read tool で画像をチャットに送って「見せた」つもりにしない。focus mode ではチャット上の画像は見えず、ユーザーには何も伝わらない
- **NEVER**: 「画像はこっちに見せて」「見せて」「画面確認したい」系の指示を、PR publish や Read tool での表示で代替しない。必ず `show.sh` を通す
...
4. **ユーザーへの伝達（必須）**: `show.sh` 実行後に「`/tmp/claude-shots/<TOPIC>/` を Preview で開きました」と通知する。Read tool で画像を表示しても focus mode では見えないので、必ず `show.sh` で `open` を通すこと。Read で送って「見せた」と判断するのは禁止
